    ...
    <div class=" max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
      <div class="p-8">
        <div class="text-center mb-8">
          ...
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white">ផ្ទៀងផ្ទាត់ អត្ថសញ្ញាណបុគ្គលិក</h2>
          <p class="text-gray-500 dark:text-gray-400 mt-2 text-sm">
            សូមបំពេញលេខកូដសម្គាល់កាត ឬលេខអត្តសញ្ញាណប័ណ្ណសញ្ជាតិខ្មែរ
          </p>
        </div>
...
const submitVerification = async () => {
  ...
  loading.value = true;
  try {
    const response = await fetch(`${environment.apiBaseUrl}staff/${staffId.value}/${vId}`, {
      method: 'GET',
      headers: { 'Accept': 'application/json' }
    });

    if (response.ok) {
      router.push({ path: `/staff-detail-view/${staffId.value}`, query: { verificationId: vId } });
    } else {
